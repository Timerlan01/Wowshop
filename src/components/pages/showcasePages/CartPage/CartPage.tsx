...
const CartPage: React.FC = () => {
  ...
  // Создаем массив товаров в корзине, фильтруя null значения
  const cartProducts: ProductCartItem[] = cart
    .map((cartItem) => {
      const product = products.find((product) => product.id === cartItem.productId);

      if (!product) return null; // Если продукт не найден, возвращаем null
    ...
    .filter((item): item is ProductCartItem => item !== null); // Фильтруем null значения
